...
/**
 * /fly [player] — Toggles flight mode for a player.
 */
public class FlyCommand implements TabExecutor {

    private final KlassenPlugin plugin;

    public FlyCommand(KlassenPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (!sender.hasPermission("klassenplugin.fly")) {
        ...
        String key = newState ? "fly-enabled" : "fly-disabled";
        target.sendMessage(KlassenPlugin.colorizeComponent(plugin.getMessage(key)));

        if (!target.equals(sender)) {
            String otherKey = newState ? "fly-enabled-other" : "fly-disabled-other";
            String msg = plugin.getMessage(otherKey).replace("{player}", target.getName());
            sender.sendMessage(KlassenPlugin.colorizeComponent(msg));
        }
        return true;
    }
