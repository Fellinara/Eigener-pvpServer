...
public class ShopManager {
    ...
    static {
        ...
        CATEGORIES.put("Erze", new String[]{
            ...
            // ── Processed minerals & blocks ──
            "COAL","COAL_BLOCK","CHARCOAL",
            ...
            "NETHERITE_SCRAP","NETHERITE_INGOT","ANCIENT_DEBRIS",
    ...
    /**
     * Authoritative default price list.  Each entry is {@code {material, buyPrice, sellPrice}}.
     * Used by {@link #createDefaultShop()} (fresh install) and
     * {@link #migrateFullShopDefaults()} (existing installations).
     */
    private static final Object[][] SHOP_DEFAULTS = {
        // ── Precious minerals ──
        {"DIAMOND",100.0,50.0},{"DIAMOND_ORE",220.0,110.0},{"DEEPSLATE_DIAMOND_ORE",240.0,120.0},{"DIAMOND_BLOCK",800.0,400.0},
        {"EMERALD",50.0,25.0},{"EMERALD_ORE",160.0,80.0},{"DEEPSLATE_EMERALD_ORE",170.0,85.0},{"EMERALD_BLOCK",400.0,200.0},
        {"NETHERITE_INGOT",500.0,250.0},{"NETHERITE_SCRAP",120.0,60.0},{"ANCIENT_DEBRIS",400.0,200.0},
        // ── Metals ──
        {"GOLD_INGOT",35.0,18.0},{"GOLD_NUGGET",5.0,2.0},{"GOLD_BLOCK",280.0,140.0},
        {"GOLD_ORE",32.0,16.0},{"DEEPSLATE_GOLD_ORE",35.0,17.0},{"NETHER_GOLD_ORE",25.0,12.0},{"RAW_GOLD",20.0,10.0},
        {"IRON_INGOT",20.0,10.0},{"IRON_NUGGET",3.0,1.0},{"IRON_BLOCK",160.0,80.0},
        {"IRON_ORE",12.0,6.0},{"DEEPSLATE_IRON_ORE",13.0,6.5},{"RAW_IRON",12.0,6.0},
        {"COPPER_INGOT",12.0,6.0},{"COPPER_BLOCK",90.0,45.0},
        {"RAW_COPPER",8.0,4.0},{"COPPER_ORE",6.0,3.0},{"DEEPSLATE_COPPER_ORE",7.0,3.5},
        // ── Coal & Redstone ──
        {"COAL",8.0,4.0},{"COAL_BLOCK",64.0,32.0},{"CHARCOAL",5.0,2.0},
        {"COAL_ORE",5.0,2.5},{"DEEPSLATE_COAL_ORE",6.0,3.0},
        {"REDSTONE",10.0,5.0},{"REDSTONE_BLOCK",80.0,40.0},
        {"REDSTONE_ORE",8.0,4.0},{"DEEPSLATE_REDSTONE_ORE",9.0,4.5},
        // ── Lapis & Quartz ──
        {"LAPIS_LAZULI",15.0,7.0},{"LAPIS_BLOCK",120.0,60.0},
        {"LAPIS_ORE",12.0,6.0},{"DEEPSLATE_LAPIS_ORE",13.0,6.5},
        {"QUARTZ",10.0,5.0},{"QUARTZ_BLOCK",80.0,40.0},{"SMOOTH_QUARTZ",90.0,45.0},
        {"NETHER_QUARTZ_ORE",8.0,4.0},
        // ── Amethyst ──
        ...
        {"GLOWSTONE",15.0,7.0},{"NETHER_WART",12.0,6.0},
        ...
        {"ROTTEN_FLESH",1.0,0.5},{"LEATHER",15.0,7.0},
        ...
        {"WET_SPONGE",35.0,17.0},{"GLOWSTONE_DUST",12.0,6.0},
    ...
    private void createDefaultShop() {
        ...
        for (Object[] d : SHOP_DEFAULTS) {
            cfg.set("items." + d[0] + ".buy", d[1]);
            cfg.set("items." + d[0] + ".sell", d[2]);
        }
    ...
    /**
     * One-time migration that ensures EVERY item listed in {@link #SHOP_DEFAULTS}
     * (= all items shown in the shop GUI categories) has a valid buy AND sell price.
     *
     * <p>This fixes existing {@code shop.yml} files that were created by older
     * plugin versions where non-ore items had {@code buy=-1} (sell-only), or were
     * missing entirely.  Items that already have a positive buy price are left
     * untouched so admin customisations are preserved.
     */
    private void migrateFullShopDefaults() {
        if (shopConfig.getBoolean("migrated-full-defaults", false)) return;

        boolean changed = false;
        for (Object[] row : SHOP_DEFAULTS) {
            String key = ((String) row[0]).toUpperCase();
            double defaultBuy  = (double) row[1];
            double defaultSell = (double) row[2];
            double[] p = items.get(key);
            if (p == null) {
                items.put(key, new double[]{defaultBuy, defaultSell});
                changed = true;
            } else {
                if (p[0] < 0) { p[0] = defaultBuy;  changed = true; }
                if (p[1] < 0) { p[1] = defaultSell; changed = true; }
            }
        }
        shopConfig.set("migrated-full-defaults", true);
        if (changed) {
            Bukkit.getScheduler().runTaskAsynchronously(plugin, this::save);
        }
    }
